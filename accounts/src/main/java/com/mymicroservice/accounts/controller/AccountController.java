...
import com.mymicroservice.accounts.dto.ResponseDto;
...
@Validated
@Tag(name = "CRUD REST APIs for Accounts in Bank Application",
    description = "CRUD REST APIs in Bank to CREATE,UPDATE,FETCH,DELETE accounts Details")
public class AccountController {
    ...
    @Operation(summary = "Fetch Account Detail REST API"
            ,description = "REST API to fetch data customer & Account Details based on a mobile number")
    ...
    @GetMapping("/fetch")
    public ResponseEntity<CustomerDto> viewCustomer(@RequestParam
    ...
    @Operation(summary = "Update Account Detail REST API",
    ...
    public ResponseEntity<ResponseDto> updateAccount(@Valid @RequestBody CustomerDto customerDto) {
        boolean updateStatus = iAccountsService.updateAccount(customerDto);
    ...
    public ResponseEntity<ResponseDto> deleteAccount(@RequestParam
                                                         @Pattern(regexp = "^$|[0-9]{10}",message = "Mobile number must be 10 digit")
                                                         String mobileNumber) {
        boolean deleteStatus = iAccountsService.deleteAccount(mobileNumber);
        return deleteStatus ? ResponseEntity.status(HttpStatus.OK)
                .body(new ResponseDto(AccountsConstants.STATUS_200, AccountsConstants.MESSAGE_200))
                : ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).
    ...
    public ResponseEntity<String> getBuildInfo() throws TimeoutException {
       ...
       //s return ResponseEntity.status(HttpStatus.OK).body(buildVersion);
    }

    public ResponseEntity<String> getBuildInfoFallBack(Throwable throwable) {
        logger.debug("getBuildInfoFallBack fallback method invoked");
